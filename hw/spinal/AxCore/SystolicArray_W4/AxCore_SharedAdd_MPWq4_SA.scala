...
case class AxCore_SharedAdd_MPWq4_SA(
  ...
  val io = new Bundle {
    ...
    val Result       = out Vec(Vec(Bits(TotalWidth bits), PECol), TileCol)  simPublic()  // Pass Result Down to Accumulate
  }
  noIoPrefix()
  ...
  for (tr <- 0 until TileRow) {
    for (pr <- 0 until PERow) {
      ...
      PreAddGroup(tr)(pr).io.Wq_FmtSel := io.Wq_FmtSel
    }
  }
...
object AxCore_SharedAdd_MPWq4_SA_Gen extends App {
  val TileNum = 16
  val PENum   = 4
  ...
  Config.setGenSubDir(s"/MPWq4_SA/AxCore_MPWq4_E${ExpoWidth}M${MantWidth}_SA_T${TileNum}P${PENum}")
  Config.spinal.generateVerilog(AxCore_SharedAdd_MPWq4_SA(
